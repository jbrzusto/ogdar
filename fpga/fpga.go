...
type Control struct {
	Command uint32 `Command Register: bit[0]: arm trigger; bit[1]: reset`

	TrigSource uint32 `Trigger source: 0: don't trigger; 1:
           trigger immediately upon arming; 2: radar trigger pulse; 3:
           ACP pulse; 4: ARP pulse`

	NumSamp uint32 `Number of Samples: number of samples to write
	   after being triggered.  Must be even and in the range
	   2...16384.`

	DecRate uint32 `Decimation Rate: number of input samples to
	   consume for one output sample. 0...65536.  For rates 1, 2,
	   3 and 4, samples can be summed instead of decimated.  For
	   rates 1, 2, 4, 8, 64, 1024, 8192 and 65536, samples can be
	   averaged instead of decimated bits [31:17] - reserved`

	Options DigdarOption `Options: digdar-specific options; see
           type DigdarOption bit[0]: Average samples; bit[1]: Sum
           samples; bit[2]: Negate video; bit[3]: Counting mode`

	TrigThreshExcite uint32 `Trigger Excite Threshold: Trigger
	   pulse is detected after trigger channel ADC value meets or
	   exceeds this value (in direction away from
	   the Trigger Relax Threshold).  -8192...8191`

	TrigThreshRelax uint32 `Trigger Relax Threshold: After a
	   trigger pulse has been detected, the trigger channel ADC
	   value must meet or exceeds this value (in direction away
	   from the Trigger Excite Threshold) before a trigger will be
	   detected again.  (Serves to debounce signal in Schmitt
	   trigger style).  -8192...8191`

	TrigDelay uint32 `Trigger Delay: How long to wait after
	   trigger is detected before starting to capture samples from
	   the video channel.  The delay is in units of ADC clocks;
	   i.e. the value is multiplied by 8 nanoseconds.`
	...
	TrigLatency uint32 `Trigger Latency: how long to wait after
	   trigger relaxation before allowing next excitation.  To
	   further debounce the trigger signal, we can specify a
	   minimum wait time between relaxation and excitation.
	   0...65535 (which gets multiplied by 8 nanoseconds)`

	ACPThreshExcite uint32 `ACP Excite Threshold: the AC Pulse is
	   detected when the ACP channel value meets or exceeds this
	   value (in direction away from the ACP Relax Threshold).
	   -2048...2047`

	ACPThreshRelax uint32 `ACP Relax Threshold: After an ACP has
	   been detected, the acp channel ADC value must meet or
	   exceeds this value (in direction away from
	   acp_thresh_excite) before an ACP will be detected again.
	   (Serves to debounce signal in Schmitt trigger style).
	   -2048...2047`

	ACPLatency uint32 `ACP Latency: how long to wait after ACP
           relaxation before allowing next excitation.  To further
           debounce the acp signal, we can specify a minimum wait time
           between relaxation and excitation.  0...1000000 (which gets
           multiplied by 8 nanoseconds)`

	ARPThreshExcite uint32 `ARP Excite Threshold: the AR Pulse is
	   detected when the ARP channel value meets or exceeds this
	   value (in direction away from the ARP Relax Threshold).
	   -2048..2047`

	ARPThreshRelax uint32 `ARP Relax Threshold: After an ARP has
	   been detected, the acp channel ADC value must meet or
	   exceeds this value (in direction away from
	   acp_thresh_excite) before an ARP will be detected again.
	   (Serves to debounce signal in Schmitt trigger style).
	   -2048..2047`

	ARPLatency uint32 `ARP Latency: how long to wait after ARP
           relaxation before allowing next excitation.  To further
           debounce the acp signal, we can specify a minimum wait time
           between relaxation and excitation.  0...1000000 (which gets
           multiplied by 8 nanoseconds)`
...
type Metadata struct {
	TrigClock      uint64 `Trigger Clock: ADC clock count at last trigger pulse`
	TrigPrevClock  uint64 `Previous Trigger Clock: ADC clock count at previous trigger pulse`
	ACPClock       uint64 `ACP Clock: ADC clock count at last ACP`
	ACPPrevClock   uint64 `Previous ACP Clock: ADC clock count at previous ACP`
	ARPClock       uint64 `ARP Clock: ADC clock count at last ARP`
	ARPPrevClock   uint64 `Previous ARP Clock: ADC clock count at previous ARP`
	TrigCount      uint32 `Trigger Count: number of trigger pulses detected since last reset`
	ACPCount       uint32 `ACP Count: number of Azimuth Count Pulses detected since last reset`
	ARPCount       uint32 `ARP Count: number of Azimuth Return Pulses (rotations) detected since last reset`
	ACPPerARP      uint32 `count of ACP between two most recent ARP`
	ADCCounter     uint32 `ADC Counter: 14-bit ADC counter used in counting mode`
	ACPAtARP       uint32 `ACP at ARP: ACP count at most recent ARP`
	ACPOffsetAtARP uint32 `ACP Offset at ARP: count of ADC clocks since last ACP, at last ARP`
	TrigAtARP      uint32 `Trig at ARP: Trigger count at most recent ARP`
}

// Live are unbuffered miscellaneous registers
type Live struct {
...
type Regs struct {
	...
	Live                              // Live Registers
	AtTrig Metadata                   // Metadata saved at last captured trigger pulse
}
...
func (fpga *FPGA) Arm() {
	fpga.Options = DDOPT_NEGATE_VIDEO | DDOPT_USE_SUM

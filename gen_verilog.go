...
func (reg reg) Pulser() string {
	...
	case reflect.Uint32:
		fallthrough
	case reflect.Int32:
		return fmt.Sprintf("        %s <= {32{addr[19:0] == `OFFSET_%-25s}} & wdata[32-1: 0];\n", reg.regname, reg.name)
	case reflect.Uint64:
		fallthrough
	case reflect.Int64:
		return fmt.Sprintf("        %s[32-1: 0] <= {32{addr[19:0] == `OFFSET_%-25s}} & wdata[32-1: 0];\n", reg.regname, reg.name+"_LO") +
			fmt.Sprintf("        %s[64-1:32] <= {32{addr[19:0] == `OFFSET_%-25s}} & wdata[32-1: 0];\n", reg.regname, reg.name+"_HI")
	}
	return ""
}
...
func main() {
	...
	fmt.Fprint(f, "// memory map definitions - generated by ogdar.go\n\n")
	...
	fmt.Fprint(f, "// register definitions - generated by ogdar.go\n\n")
	for i := 0; i < len(regs); i++ {
	...
	fmt.Fprint(f, "// getter logic - generated by ogdar.go\n\n")
	for i := 0; i < len(regs); i++ {
	...
	fmt.Fprint(f, "// setter logic - generated by ogdar.go\n\n")
	for i := 0; i < len(regs); i++ {
	...
	fmt.Fprint(f, "// pulser logic - generated by ogdar.go\n\n")
	for i := 0; i < len(regs); i++ {
